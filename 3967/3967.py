...
for date in dates:
    ...
    year = date.year
    dest_path = "s3://staging-near-data-analytics/shashwat/ps-3967/poi_footfall/{}/".format(date)
...
df = spark.read.parquet('s3://staging-near-data-analytics/shashwat/ps-3967/poi_footfall/*/*')
...
df.coalesce(1).write.mode('overwrite').csv(f"s3://staging-near-data-analytics/shashwat/ps-3967/bayer_june/", header = True)

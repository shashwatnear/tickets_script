...
for date in dates:
    try:
        ...
        year = date.year
        dest_path = "s3://staging-near-data-analytics/shashwat/smrt/june/footfall_gh8/50m/{}/".format(date)
        print(dest_path)
...
df = spark.read.parquet('s3://staging-near-data-analytics/shashwat/smrt/june/footfall_gh8/50m/*/*')
print("Before")
...
final.write.mode('append').parquet("s3://staging-near-data-analytics/shashwat/smrt/june/footfall_gh8/50m/combined/")


# ######################### Profile #####################
# report 1
df = spark.read.parquet('s3://staging-near-data-analytics/shashwat/smrt/june/footfall_gh8/50m/combined/*')
demog_df = spark.read.csv('s3://near-data-analytics/Ajay/coreSeg.csv', header=True)
...
final.coalesce(1).write.mode('append').csv("s3://staging-near-data-analytics/shashwat/smrt/june/reports/report_1/", header = True)
...
result.coalesce(1).write.mode('append').csv("s3://staging-near-data-analytics/shashwat/smrt/june/reports/report_2/", header=True)
...
df.coalesce(1).write.mode('append').csv("s3://staging-near-data-analytics/shashwat/smrt/june/reports/report_3/", header=True)

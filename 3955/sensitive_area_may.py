...
for date in dates:
    ...
    ff_df.write.mode("append").parquet(f"s3://staging-near-data-analytics/shashwat/ps-3955/rule_engine_may/footfall/{date}/")

for date in dates:
    try:
        ref = spark.read.parquet(f's3://staging-near-data-analytics/shashwat/ps-3955/rule_engine_may/footfall/{date}/*')
        ref = ref.withColumn('date', lit(date))
        ref = ref.groupBy('date').agg(countDistinct('aspkId').alias('ids'))
        ref.write.mode('overwrite').csv(f's3://staging-near-data-analytics/shashwat/ps-3955/intermediate/rule_engine_may/{date}/', header=True)
    except Exception as e:
        print(f"Error processing date {date}: {e}")

# Now read all intermediate files and combine them
intermediate_files = 's3://staging-near-data-analytics/shashwat/ps-3955/intermediate/rule_engine_may/*'
result = spark.read.csv(intermediate_files, header=True)
result = result.orderBy('date')

result.coalesce(1).write.mode('append').csv(f's3://staging-near-data-analytics/shashwat/ps-3955/final/rule_engine_may/', header = True)

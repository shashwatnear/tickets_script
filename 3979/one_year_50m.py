...
#             df = df.select(['ifa', df.geoHash9.alias('geohash')])
#             df = df.withColumn('data_partner', lit(dp))
#             ff_df = df.join(poi, on='geohash', how='inner')
#             ff_df = ff_df.select('ifa', 'data_partner')
#             ff_df.write.mode("append").parquet(f"s3://staging-near-data-analytics/shashwat/ps-3979/one_year_50m/stay/rtb/{dp}/{date}/")
#         except:
...
#             ff_df = df.join(poi, on='geohash', how='inner')
#             ff_df = ff_df.select('ifa', 'data_partner')
#             ff_df.write.mode("append").parquet(f"s3://staging-near-data-analytics/shashwat/ps-3979/one_year_50m/stay/sdk/{dp}/{date}/")
#         except:
#             print(date, " Path does not exists")
...
for date in dates:
    try:
        ...
        rule = rule.select(['aspkId', rule.GH8.alias('geohash')])
        rule = rule.join(poi, on='geohash', how='inner')
        rule = rule.select('aspkId')
        rule.write.mode("append").parquet(f"s3://staging-near-data-analytics/shashwat/ps-3979/one_year_50m/rule_engine/{date}/")
    except:
        print(date, " Path does not exists")

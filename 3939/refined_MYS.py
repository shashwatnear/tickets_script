...
for date in dates:
    try:
        ...
        data = spark.read.parquet(f"s3://near-data-warehouse/refined/dataPartner=*/year=2024/month=05/day={day}/hour=*/country=MYS/*")
        data = data.select(['ifa', data.geoHash9.substr(1,8).alias("geohash"), 'coreSeg', 'eventDTLocal'])
        data.write.mode("append").parquet(f"s3://staging-near-data-analytics/shashwat/ps-3939/refined/MYS/{date}")
    except:

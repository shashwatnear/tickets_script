...
# df.write.mode("append").parquet("s3://staging-near-data-analytics/shashwat/ps-3939/parents_at_kinder_pre/parquet/")
df = df.groupBy('name_en').agg(countDistinct('deviceID').alias('parents_count'))
df.coalesce(1).write.mode('append').csv('s3://staging-near-data-analytics/shashwat/ps-3939/parents_at_kinder_pre/csv/',header=True)

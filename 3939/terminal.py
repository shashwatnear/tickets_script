...
# df.write.mode("append").parquet("s3://staging-near-data-analytics/shashwat/ps-3939/airport_terminal/parquet/")
df = df.groupBy('name_en').agg(countDistinct('deviceID').alias('people_count'))
df.coalesce(1).write.mode('append').csv('s3://staging-near-data-analytics/shashwat/ps-3939/airport_terminal/csv/',header=True)

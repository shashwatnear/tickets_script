...
poi = spark.read.csv('s3://near-data-analytics/shashwat/bayer/SMRT-Dashboard location master v1 - Sheet1.csv', header = True)
...
for date in dates:
    try:
        ...
        year = date.year
        dest_path = "s3://staging-near-data-analytics/shashwat/bayer/june/footfall_gh8/50m/{}/".format(date)
...
# # # #########################
# report 1
df = spark.read.parquet('s3://staging-near-data-analytics/shashwat/bayer/june/footfall_gh8/50m/combined/*')
...
final.coalesce(1).write.mode('append').csv("s3://staging-near-data-analytics/shashwat/bayer/june/reports/report_1/", header = True)
...
gender = result.where(col('Profile').isin(['Male', 'Female']))
gender = gender.withColumnRenamed('Profile', 'Gender')

age = result.where(col('Profile').isin(['18-24', '25-34', '35-44', '45-54', '55+']))
age = age.withColumnRenamed('Profile', 'Age')

result = gender.join(age, on = 'deviceID', how = 'inner')
result.coalesce(1).write.mode('append').csv("s3://staging-near-data-analytics/shashwat/bayer/june/reports/report_2/", header=True)

# # # #########################
# report 3
df = spark.read.parquet('s3://staging-near-data-analytics/shashwat/bayer/june/footfall_gh8/50m/combined/*')
df = df.withColumn('Month', lit('July'))
...
df.coalesce(1).write.mode('append').csv("s3://staging-near-data-analytics/shashwat/bayer/june/reports/report_3/", header=True)

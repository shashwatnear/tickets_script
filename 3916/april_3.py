...
spark = SparkSession.builder.appName("3916_april_3").getOrCreate()
sc = spark.sparkContext

# File 1
df = spark.read.parquet('s3://staging-near-data-analytics/shashwat/ps-3916/April_Data/50m/3rd_week/*/*').withColumnRenamed("Province", "District").withColumnRenamed("City", "Province")
...
result_df.coalesce(1).write.option("header",True).mode('append').csv("s3://staging-near-data-analytics/shashwat/ps-3916/arpil_report_final/april/50m/3rd_week/File_1/")
...
final.coalesce(1).write.option("header",True).mode('append').csv("s3://staging-near-data-analytics/shashwat/ps-3916/arpil_report_final/april/50m/3rd_week/File_4/")
...
final_result.coalesce(1).write.option("header",True).mode('append').csv("s3://staging-near-data-analytics/shashwat/ps-3916/arpil_report_final/april/50m/3rd_week/File_2")
...
final_result = final_result.withColumn("poi_no", final_result["poi_no"].cast("int"))
final_result = final_result.orderBy("poi_no")
final_result.coalesce(1).write.option("header",True).mode('append').csv("s3://staging-near-data-analytics/shashwat/ps-3916/arpil_report_final/april/50m/3rd_week/File_3")

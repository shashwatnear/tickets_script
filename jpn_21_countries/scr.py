...
# datelist = pd.date_range(end=yesterday, periods=1).tolist()
...
for country in countries:
    for date in dates:
        try:
            ...
            data_footfall.write.mode("append").parquet("s3://staging-near-data-analytics/Adithya/3732/rule_engg/{}/{}/".format(country,date))
...
countries1 = ['MYS', 'NLD', 'NZL', 'PHL', 'SAU', 'SGP', 'THA', 'TUR','VNM','IND','IDN','ARE','IRL','DEU','FRA','GBR','ESP','HKG','ITA','AUS','TWN','CAN','MEX']
for country in countries1:
    new = spark.read.parquet("s3://staging-near-data-analytics/Adithya/3732/rule_engg/{}/*/*".format(country)).dropDuplicates()
    ...
    data1.write.mode('append').parquet("s3://staging-near-data-analytics/Adithya/3732/Tourist_card/{}/Tourist_90_data/{}".format(currentdate, country))
    

df = spark.read.parquet("s3://staging-near-data-analytics/Adithya/3732/Tourist_card/{}/Tourist_90_data/*/*".format(currentdate))
df1 = df.filter(col('HomeLocation')!="Japan")

df_1 = df1.filter(col('new_days')>=1).withColumnRenamed('HomeLocation','country').filter((col('country').isin(['MYS', 'NLD', 'NZL', 'PHL', 'SAU', 'SGP', 'THA', 'TUR','VNM','IND','IDN','ARE','IRL','DEU','FRA','GBR','ESP','HKG','ITA','AUS','TWN','CAN','MEX'])))

card_info = spark.read.csv("s3://near-data-analytics/adithya/tourist_card_xl_jpn.csv", header=True).select('seg_id','country').dropDuplicates()

data1 = df_1.join(card_info, on='country', how='inner').dropDuplicates()

data1.select('aspkId','seg_id').coalesce(1).write.partitionBy('seg_id').mode('append').csv("s3://staging-near-data-analytics/Adithya/3732/Tourist_card/{}/Tourist_card_seg/".format(currentdate))


# a = ['aws', 's3api', 'list-objects', '--bucket', 'staging-near-data-analytics' ,'--prefix', 'Adithya/3732/Tourist_card/{}/Tourist_card_seg/'.format(currentdate), '--output', 'text', '--query', 'Contents[].{Key: Key}']
...
# fin=[]
# seg_ids = [69993, 69992, 69991, 69990, 69989, 69988, 69987, 69985, 69984, 69983, 69982, 69981, 69980, 69979, 69978, 69977, 69976, 69975, 69974, 69973, 72127, 75828, 75829]

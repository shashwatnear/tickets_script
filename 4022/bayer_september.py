...
df = df.groupBy(['name', 'category', 'top_category', 'telephone', 'id', 'neighbourhood', 'sent', 'building_door_no', 'City', 'Province', 'address', 'lon', 'lat']).agg(countDistinct('ifa').alias('FOOTFALL_SEPTEMBER'))
df.coalesce(1).write.mode('overwrite').csv(f"s3://staging-near-data-analytics/shashwat/ps-4022/bayer_september/", header = True)

...
def get_geohash(lat, lon):
    a = proximityhash.create_geohash(float(lat), float(lon), 500, 8)
    return a
...
for date in dates:
    try:
        ...
        year = date.year
        dest_path = "s3://staging-near-data-analytics/shashwat/ps-3932/final/staypoints/footfall_gh8/500m/{}/".format(date)
...
# # #########################
# # report 1
# df = spark.read.parquet('s3://staging-near-data-analytics/shashwat/ps-3932/final/refined/250m/footfall_gh8/*/*')
...
# final.coalesce(1).write.mode('overwrite').csv("s3://staging-near-data-analytics/shashwat/ps-3932/final/report_gh8/250m/report_1/", header = True)
...
# result.coalesce(1).write.mode('overwrite').csv("s3://staging-near-data-analytics/shashwat/ps-3932/final/report_gh8/250m/report_2/", header=True)
...
# df.coalesce(1).write.mode('overwrite').csv("s3://staging-near-data-analytics/shashwat/ps-3932/final/report_gh8/250m/report_3/", header=True)

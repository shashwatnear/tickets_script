...
input_ip = spark.read.csv('s3://staging-near-data-analytics/shashwat/ps-4001/files/list3_clean.csv', header = True)

df1 = spark.read.csv('s3://staging-near-data-analytics/shashwat/ps-4001/maxmind_data/3.csv', header = True).dropDuplicates()
...
df2 = spark.read.option("header", "true").option("delimiter", ";").csv('s3://staging-near-data-analytics/shashwat/ps-4001/de_data/azira_results_3.csv').dropDuplicates()

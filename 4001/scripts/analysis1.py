...
input_ip = spark.read.csv('s3://staging-near-data-analytics/shashwat/ps-4001/files/list1.csv', header = True)

# print("input IPs count")
# input_ip.agg(count("ip")).show()

df1 = spark.read.csv('s3://staging-near-data-analytics/shashwat/ps-4001/maxmind_data/1.csv', header = True)
...
df2 = spark.read.option("header", "true").option("delimiter", ";").csv('s3://staging-near-data-analytics/shashwat/ps-4001/de_data/azira_results_1.csv')
...
# IP Counts
total_ips = 112295  # total number of IPs
...
df1_lower = df1.withColumn('country_name', lower(col('country_name'))) \
               ...
               .withColumn('country_iso_code', lit('usa')) \
...
df_joined = df1_lower.join(df2_lower, on='ip', how='inner')

# Compare countries (same, different assignments)
df_countries_same = df_joined.filter(col('country_iso_code') == col('pulseplus-country')).count()
df_countries_different = df_joined.filter(col('country_iso_code') != col('pulseplus-country')).count()

# Compare states (same, different assignments)
df_states_same = df_joined.filter(col('subdivision_iso_code') == col('pulseplus-region')).count()
df_states_different = df_joined.filter(col('subdivision_iso_code') != col('pulseplus-region')).count()

# Compare cities (same, different assignments)
# df_cities_same = df_joined.filter(col('city_name') == col('pulseplus-city')).count()
# df_cities_different = df_joined.filter(col('city_name') != col('pulseplus-city')).count()
...
df_postals_same = df_joined.filter(col('postal_code') == col('pulseplus-postal-code')).count()
df_postals_different = df_joined.filter(col('postal_code') != col('pulseplus-postal-code')).count()

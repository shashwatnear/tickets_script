...
def normal_pixel(start, end, datasource_ids):
    ...
    # folder_count_df = folder_count_df.filter(folder_count_df.datasource_id != 3193)
    folder_count_df.select("Date", "datasource_id", "impression_count").write.mode('append').parquet(f"s3://staging-near-data-analytics/shashwat/sojern_pixel_counts/single_day/Base_data/normal_pixel/{range_folder}/")





# def universal_pixel(start, end):
#     dates = get_dates_between(start, end)
#     range_folder = "{}_{}".format(start.replace("-", ""), end.replace("-", ""))
#     for date in dates:
#         try:
...
#             month = '{:02d}'.format(date.month)
#             year = date.year 
#             path = f"s3://near-pixelmanager/nifi/country=AUS/id=d99529d9/datasource_id=3193/year={year}/month={month}/day={day}/"
#             print(path)
#             dfu = spark.read.json(path)
#             # Add extra column "Date" to dfp with values extracted from the folder path
#             dfu = dfu.withColumn("Date", lit(f"{year}-{month}-{day}"))
#             #dfu = dfu.withColumn("ut20_array", split(col("query_params.ut20"), ",")) \
#              #   .withColumn("ut20", when(col("query_params.ut20").isNull(), array(lit("N/A"))).otherwise(col("ut20_array")))
#             #dfu = dfu.withColumn("ut20", explode("ut20"))
#             dfu = dfu.withColumn("ut1", explode(split(col("query_params.ut1"), ",")))
#             dfu = dfu.withColumn("ut2", explode(split(col("query_params.ut2"), ",")))
#             dfu = dfu.withColumn("ut3", explode(split(col("query_params.ut3"), ",")))
#             dfu.select("datasource_id",  "Date", "ut1", "ut2", "ut3", "istts").write.mode('append').parquet(f"s3://staging-near-data-analytics/shashwat/sojern_pixel_counts/Base_data/universal_pixel/{range_folder}/")
#         except Exception as e:
#             print(f"Skipping {path} due to error: {type(e).__name__}: {e}")
...
def process_mongodb():
    ...
    df_filtered.coalesce(1).write.mode("overwrite").json("s3://staging-near-data-analytics/shashwat/sojern_pixel_counts/single_day/coarse_1/")
    # spark_mongo.stop()
...
def process(start, end):
    ...
    df2 = spark.read.parquet(f"s3://staging-near-data-analytics/shashwat/sojern_pixel_counts/single_day/Base_data/normal_pixel/{range_folder}/")
    df2 = df2.select("Date", "datasource_id", "impression_count")
    #------------------------read mongo db data --------------------------
    df3 = spark.read.json("s3://staging-near-data-analytics/shashwat/sojern_pixel_counts/single_day/coarse_1/")
    ...
    df_union.coalesce(1).write.mode('append').option("header", "true").csv(f"s3://staging-near-data-analytics/shashwat/sojern_pixel_counts/single_day/final_output/{range_folder}/",header=True)
    spark.stop()

...
# for sgp we take 30 days of data
...
for country in countries:
    for date in dates:
        try:
            ...
            data_footfall.write.mode("append").parquet("s3://staging-near-data-analytics/tourist_dags_data/sgp/rule_engg/{}/{}/".format(country,date))
...
sgp = spark.read.parquet("s3://staging-near-data-analytics/tourist_dags_data/sgp/rule_engg/SGP/*/*").dropDuplicates()
...
for country in countries:
    new = spark.read.parquet("s3://staging-near-data-analytics/tourist_dags_data/sgp/rule_engg/{}/*/*".format(country)).dropDuplicates()
    ...
    data1.write.mode('append').parquet("s3://staging-near-data-analytics/tourist_dags_data/sgp/Tourist_card/{}/Tourist_90_data/{}".format(currentdate, country))

###############################
    
df = spark.read.parquet("s3://staging-near-data-analytics/tourist_dags_data/sgp/Tourist_card/{}/Tourist_90_data/*/*".format(currentdate))
...
data1.select('aspkId','segment_id').coalesce(1).write.partitionBy('segment_id').mode('append').csv("s3://staging-near-data-analytics/tourist_dags_data/sgp/Tourist_card/{}/Tourist_card_seg/".format(currentdate))

###############################


a = ['aws', 's3api', 'list-objects', '--bucket', 'staging-near-data-analytics' ,'--prefix', 'tourist_dags_data/sgp/Tourist_card/{}/Tourist_card_seg/'.format(currentdate), '--output', 'text', '--query', 'Contents[].{Key: Key}']
...
fin=[]
seg_ids = [90830,90831,90828,90829,90827,90826,90825,90824,90823,90822,90821,90820,90818,90819,90817,90816,90815,90814,90812,90813,90810,90811,90809,90808]

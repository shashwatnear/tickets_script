...
for date in dates[-65:]:
	try:
		...
		data_footfall.write.mode("append").parquet(f"s3://staging-near-data-analytics/tourist_dags_data/mys/CHN/{currentdate}/refined/{date}/")
...
##########################################################################
total = spark.read.parquet(f"s3://staging-near-data-analytics/tourist_dags_data/mys/CHN/{currentdate}/refined/*/*")
...
mapped.write.mode('append').parquet(f's3://staging-near-data-analytics/tourist_dags_data/mys/CHN/{currentdate}/chinese_tourist/')

mapped = spark.read.parquet(f's3://staging-near-data-analytics/tourist_dags_data/mys/CHN/{currentdate}/chinese_tourist/*')
...
hkg_hl = spark.read.parquet(f's3://staging-near-data-analytics/tourist_dags_data/mys/CHN/{currentdate}/chinese_tourist/*').select(['aspkId']).dropDuplicates()

# print(mys_hl.select('aspkId').distinct().count()) #

not_hkg = oct_data.join(hkg_hl,on='aspkId',how='left_anti').dropDuplicates()
not_hkg.write.mode('append').parquet(f's3://staging-near-data-analytics/tourist_dags_data/mys/CHN/{currentdate}/HL_not_in_MYS/')

not_hkg = spark.read.parquet(f's3://staging-near-data-analytics/tourist_dags_data/mys/CHN/{currentdate}/HL_not_in_MYS/*')
...
sdt_ch1_cnt.write.mode('append').parquet(f's3://staging-near-data-analytics/tourist_dags_data/mys/CHN/{currentdate}/CHN_Tourists_in_MYS/')

sdt_ch1_cnt =spark.read.parquet(f's3://staging-near-data-analytics/tourist_dags_data/mys/CHN/{currentdate}/CHN_Tourists_in_MYS/*')
...
df = spark.read.parquet(f's3://staging-near-data-analytics/tourist_dags_data/mys/CHN/{currentdate}/CHN_Tourists_in_MYS/*')
print(df.select('aspkId').distinct().count())
final = df.select('aspkId').distinct()
final.coalesce(1).write.option("header",True).mode('append').csv(f's3://staging-near-data-analytics/tourist_dags_data/mys/CHN/{currentdate}/final_ifas/')

# ######################

a = ['aws', 's3api', 'list-objects', '--bucket', 'staging-near-data-analytics' ,'--prefix', 'tourist_dags_data/mys/CHN/{}/final_ifas/'.format(currentdate), '--output', 'text', '--query', 'Contents[].{Key: Key}']

...
spark = SparkSession.builder.appName("Tourist cards for Singapore").getOrCreate()
...
#             data_footfall.write.mode("append").parquet("s3://staging-near-data-analytics/shashwat/ps-3959/sgp/rule_engg/{}/{}/".format(country,date))
...
# countries = [
#     'ARE', 'AUS', 'CAN', 'DEU', 'ESP', 'FRA', 'GBR', 'HKG', 'IDN', 'IND', 'IRL',
#     'ITA', 'JPN', 'MEX', 'MYS', 'NLD', 'NZL', 'PHL', 'SAU', 'THA', 'TUR', 
#     'TWN', 'USA', 'VNM'
# ]
...
# sgp = spark.read.parquet("s3://staging-near-data-analytics/shashwat/ps-3959/sgp/rule_engg/SGP/*/*").dropDuplicates()
...
#     new = spark.read.parquet("s3://staging-near-data-analytics/shashwat/ps-3959/sgp/rule_engg/{}/*/*".format(country)).dropDuplicates()
...
#     data1.write.mode('append').parquet("s3://staging-near-data-analytics/shashwat/ps-3959/sgp/Tourist_card/{}/Tourist_90_data/{}".format(currentdate, country))

# # ###############################

# df = spark.read.parquet("s3://staging-near-data-analytics/shashwat/ps-3959/sgp/Tourist_card/{}/Tourist_90_data/*/*".format(currentdate))
# df1 = df.filter(col('HomeLocation') != "Singapore")

# df_1 = df1.filter(col('new_days') >= 1).withColumnRenamed('HomeLocation','country').filter((col('country').isin(['ARE', 'AUS', 'CAN', 'DEU', 'ESP', 'FRA', 'GBR', 'HKG', 'IDN', 'IND', 'IRL', 'ITA', 'JPN', 'MEX', 'MYS', 'NLD', 'NZL' 'PHL', 'SAU', 'THA', 'TUR', 'TWN', 'USA', 'VNM'])))
...
#     temp.coalesce(1).write.mode('overwrite').csv(f"s3://staging-near-data-analytics/shashwat/ps-3959/sgp/aspkid_to_create_cards/{country}/", header = True)

# # ###################### For Excel report ####################

# df = spark.read.parquet("s3://staging-near-data-analytics/shashwat/ps-3959/sgp/Tourist_card/{}/Tourist_90_data/*/*".format(currentdate))
# df = df.filter(col('HomeLocation') != "Singapore")

# days_diff = list(range(1, 91))
# countries = ['ARE', 'AUS', 'CAN', 'DEU', 'ESP', 'FRA', 'GBR', 'HKG', 'IDN', 'IND', 'IRL', 'ITA', 'JPN', 'MEX', 'MYS', 'NLD', 'NZL', 'PHL', 'SAU', 'THA', 'TUR', 'TWN', 'USA', 'VNM']
...
# pivoted_result.coalesce(1).write.mode('append').csv("s3://staging-near-data-analytics/shashwat/ps-3959/sgp/days_diff_by_country/", header=True)

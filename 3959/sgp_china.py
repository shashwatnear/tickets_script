...
spark = SparkSession.builder.appName("SGP_China_tourists").getOrCreate()
sc = spark.sparkContext

# ############ SGP Refined Extraction ###################
...
# 		year = date_obj.year 
# 		path = "s3://near-data-warehouse/refined/dataPartner=*/year={}/month={}/day={}/hour=*/country=SGP/*".format(year,month,day)
# 		print(path)
# 		data = spark.read.parquet(path)
# 		data_footfall = data.select(['aspkId','devLanguage','eventTs','devCarrier','eventDTLocal']).dropDuplicates()
# 		data_footfall.write.mode("append").parquet(f"s3://staging-near-data-analytics/shashwat/TouristCardsData/{currentdate}/SGP/refined/{date}/")
...
# ##########################################################################
# total = spark.read.parquet(f"s3://staging-near-data-analytics/shashwat/TouristCardsData/{currentdate}/SGP/refined/*/*")
...
# homeloc = spark.read.parquet("s3://near-datamart/homeLocation/version=v1/MasterDB/dataPartner=combined/year={}/month={}/day=*/country=SGP/*".format(current_year,Previous_month))
...
# mapped.write.mode('append').parquet(f's3://staging-near-data-analytics/shashwat/TouristCardsData/{currentdate}/SGP/chinese_tourist/')


# mapped = spark.read.parquet(f's3://staging-near-data-analytics/shashwat/TouristCardsData/{currentdate}/SGP/chinese_tourist/*')
# print("devices with HLs in SGP")
# print(mapped.select('aspkId').distinct().count()) #


# ### taking devices which are not having HLs in SGP

# sgp_hl = spark.read.parquet(f's3://staging-near-data-analytics/shashwat/TouristCardsData/{currentdate}/SGP/chinese_tourist/*').select(['aspkId']).dropDuplicates()

# # print(sgp_hl.select('aspkId').distinct().count()) #

# not_sgp = oct_data.join(sgp_hl,on='aspkId',how='left_anti').dropDuplicates()
# not_sgp.write.mode('append').parquet(f's3://staging-near-data-analytics/shashwat/TouristCardsData/{currentdate}/SGP/HL_not_in_SGP/')

# not_sgp = spark.read.parquet(f's3://staging-near-data-analytics/shashwat/TouristCardsData/{currentdate}/SGP/HL_not_in_SGP/*')
# print("devices not having HLs in SGP")
# print(not_sgp.select('aspkId').distinct().count()) #


# #### datediff logic for devices with no HL in SGP
# not_sgp = spark.read.parquet(f's3://staging-near-data-analytics/shashwat/TouristCardsData/{currentdate}/SGP/HL_not_in_SGP/*')
# data = not_sgp.withColumn('timestamp',from_unixtime("eventTs", "yyyy-MM-dd HH:mm:ss"))
...
# sdt_ch1_cnt.write.mode('append').parquet(f's3://staging-near-data-analytics/shashwat/TouristCardsData/{currentdate}/SGP/CHN_Tourists_in_SGP/')

# sdt_ch1_cnt =spark.read.parquet(f's3://staging-near-data-analytics/shashwat/TouristCardsData/{currentdate}/SGP/CHN_Tourists_in_SGP/*')
...
# df = spark.read.parquet(f's3://staging-near-data-analytics/shashwat/TouristCardsData/{currentdate}/SGP/CHN_Tourists_in_SGP/*')
# print(df.select('aspkId').distinct().count())
# final = df.select('aspkId').distinct()
# final.coalesce(1).write.option("header",True).mode('append').csv(f's3://staging-near-data-analytics/shashwat/TouristCardsData/{currentdate}/SGP/final_ifas/')
...
apikey = "3db611d5-a3ee-4a80-bfc5-f7b1e00b6e7f"

# S3 base path
s3_base_path = "s3://staging-near-data-analytics/shashwat/ps-3959/sgp/aspkid_to_create_cards"
...
def run_curl_command(country, s3_file_path):
    data = {
        ...
        "country": "SGP",  # Assuming 'MYS' is a static value as per your example

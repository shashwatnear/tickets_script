...
for country in countries:
    for date in dates:
        try:
            ...
            data_footfall.write.mode("append").parquet("s3://staging-near-data-analytics/shashwat/ps-3959/hkg/rule_engg/{}/{}/".format(country,date))
...
countries = [
    'ARE', 'AUS', 'CAN', 'DEU', 'ESP', 'FRA', 'GBR', 'IDN', 'IND', 'IRL',
    'ITA', 'JPN', 'MEX', 'MYS', 'NLD', 'NZL', 'PHL', 'SAU', 'SGP', 'THA', 'TUR', 
    'TWN', 'USA', 'VNM'
]
...
hkg = spark.read.parquet("s3://staging-near-data-analytics/shashwat/ps-3959/hkg/rule_engg/HKG/*/*").dropDuplicates()
hkg = hkg.withColumnRenamed('v_date','hkg_date')

hkg_cnts = hkg.groupBy('aspkId').agg(countDistinct('hkg_date').alias('hkg_days'))

countries1 = ['ARE', 'AUS', 'CAN', 'DEU', 'ESP', 'FRA', 'GBR', 'IDN', 'IND', 'IRL', 'ITA', 'JPN', 'MEX', 'MYS', 'NLD', 'NZL', 'PHL', 'SAU', 'SGP', 'THA', 'TUR', 'TWN', 'USA', 'VNM']

for country in countries1:
    new = spark.read.parquet("s3://staging-near-data-analytics/shashwat/ps-3959/hkg/rule_engg/{}/*/*".format(country)).dropDuplicates()
    ...
    data1.write.mode('append').parquet("s3://staging-near-data-analytics/shashwat/ps-3959/hkg/Tourist_card/{}/Tourist_90_data/{}".format(currentdate, country))

# ###############################

df = spark.read.parquet("s3://staging-near-data-analytics/shashwat/ps-3959/hkg/Tourist_card/{}/Tourist_90_data/*/*".format(currentdate))
...
for country in countries:
    ...
    temp.coalesce(1).write.mode('overwrite').csv(f"s3://staging-near-data-analytics/shashwat/ps-3959/hkg/aspkid_to_create_cards/{country}/", header = True)
...
# df = spark.read.parquet("s3://staging-near-data-analytics/shashwat/ps-3959/hkg/Tourist_card/{}/Tourist_90_data/*/*".format(currentdate))
# df = df.filter(col('HomeLocation') != "HongKong")
...
# pivoted_result.coalesce(1).write.mode('append').csv("s3://staging-near-data-analytics/shashwat/ps-3959/hkg/days_diff_by_country/", header=True)
